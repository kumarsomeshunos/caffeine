...
- **`LeaveActiveNote` is the single choke point** for committing the editor and discarding blank notes. Any new path that changes the selected note goes through it. The one exception, `SyncActiveNoteToSelection` (reached when a search filters the open note out of the list), must at least `CommitEditor()` — a filter change is not a decision to throw a draft away.
- **A prompt set is a note *kind*, not a feature.** `Note.Kind` chooses the body — a `FlowDocument` or a queue of `Prompt`s — and everything else is the ordinary note path: one list, one store, one debounce, one bin. `Note.PlainText` mirrors the prompt texts, which is what makes search, the row preview and blank-note discard work untouched. Don't fork any of them for prompt notes.
- **`RebuildPrompts` is the only thing that puts a prompt on screen,** and it runs on *structural* change only — add, delete, tick, reorder, load. Typing writes straight through to the model; a rebuild mid-keystroke takes the caret with it.
- **The prompt list's order is the model's order.** Position in the JSON array is position on screen, and every sent prompt follows every unsent one. Sending is `Remove` then `Add`. Don't add an `Order` field; `NormalisePrompts` is the single place that invariant is re-established.
- **`_promptsReadOnly` blocks writes *and* the index mirror.** A set that failed to load leaves `_prompts` empty, which is indistinguishable from a set that is genuinely empty — so `CommitEditor`, `RebuildPrompts` and the blank-note discard must all skip it, or one unreadable file costs the user the whole note.
...
- **Check `EditingCommands` before claiming a `Ctrl+Shift` chord.** `Ctrl+Shift+N` is `ToggleNumbering` and `Ctrl+Shift+L` is `ToggleBullets`; a focused `RichTextBox` consumes them before the host window's `KeyDown` ever runs, so the shortcut silently formats text instead of doing what it says. That is why "new prompt set" is `Ctrl+Shift+P`.
- **A `Path` is hit-tested against its stroke, not its bounds.** A 1.3px glyph is a 1.3px target. Wrap anything draggable or clickable in a `Border` with `Background="Transparent"` and padding — see `NotesView.BuildPromptGrip`.
- **`FindResource` in code is a one-time lookup.** Elements built in code hold brush *instances*, so swapping the theme dictionary never reaches them the way `DynamicResource` does. Anything built in code must rebuild on `ThemeManager.ThemeChanged` (`NotesView` does; `TodoView` is logged debt).
- **Folding to `Height="0"` is not hiding.** The children stay in the tab order and the automation tree, where they act on something the user cannot see. Use `Visibility="Collapsed"` when they should be genuinely gone.
...
- Anything holding user content saves atomically (temp file then `File.Move(overwrite: true)`) and never overwrites a file it failed to parse — quarantine it instead. A failed save must surface in the UI, never silently. If the quarantine itself fails the file is still there, so refuse the write rather than destroy it.
- **A store's `Save` and `Load` must use the same `JsonSerializerOptions`.** Adding a converter to one and not the other means the app cannot read the file it just wrote — and the corrupt-file path then quarantines a perfectly good one and presents an empty list. This has already cost the whole notes index once. After touching a serialiser, verify a full round trip across a **restart**.
...
- **Persistence checks must cross a restart** — write, quit, relaunch, read. A round trip inside one session shares the in-memory objects and proves nothing about what is on disk.
- The app is drivable without a test project: UI Automation reaches it from PowerShell (`AutomationId` maps to `x:Name`), and `Graphics.CopyFromScreen` will capture the window. Back up `%AppData%\Caffeine` before any run that writes, and restore it after — the user's notes are not a fixture.
...
- Vocabulary: **Standard mode** (power API) vs **Stay Green** (jiggle); **auto-off** (the caffeine countdown) vs **Pomodoro** (work/break cycles). A **prompt set** is a kind of note, holding **prompts to send** and **sent** prompts; its title is an **application name**, and the `P` tag on a row marks one. Don't mix these terms.
- Prompt sets share the Notes amber and the Notes ambience — they are a note, not a sixth feature, and must never acquire a colour identity of their own.
